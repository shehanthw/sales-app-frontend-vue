    ...
    <div class="p-5">
      <div class="flex justify-between items-start mb-4">
        <div>
          ...
          <p class="font-mono text-xs text-gray-600">{{ installment.order_id  }}</p>
        </div>
        <span
          class="bg-blue-50 text-blue-700 px-2 py-1 rounded text-[10px] font-bold uppercase border border-blue-100"
          :class="{
            'text-green-700 border-green-blue-100 bg-green-50': installment.status === 'paid'
          }"
        >
          {{ installment.status }}
        </span>
      ...
      <div class="grid grid-cols-2 gap-4 bg-gray-50 rounded-xl p-3 mb-4">
        <div>
          ...
          <p class="text-sm font-bold text-gray-700">{{ installment.due_date }}</p>
        ...
        <div class="text-right">
          ...
          <p class="text-sm font-bold text-yell-400 italic">{{ installment.paid_at ? installment.paid_at.split("T")[0] : "Not Paid" }}</p>
        </div>
      </div>
      ...
      >
        ...
        <button
          ...
          :class="{
            'bg-gray-600 cursor-not-allowed opacity-50': installment.status === 'paid'
          }"
